...
// Calculate the Pt broadening and plot in funcion of the Q2 bins
void PtBroadeningVarIntegratedSplit(char var[], TString inputDirectory, TString plotDirectory) {
  ...
  for(int nPion = 1; nPion <= N_PION; nPion++) { // Loops in every number of pion
    ...
    SetXShift(g[0][nPion-1], (- 0.02 + 0.01*nPion), nBins);
    SetXShift(g[1][nPion-1], (- 0.02 + 0.01*nPion), nBins);
    SetXShift(g[2][nPion-1], (- 0.02 + 0.01*nPion), nBins);
  ...
  for(int k = 0; k < N_STARGETS; k++) {
    g[k][0]->SetMarkerStyle(8);
    g[k][0]->SetMarkerSize(.6);
    g[k][1]->SetMarkerStyle(8);
    g[k][1]->SetMarkerSize(.6);
    g[k][2]->SetMarkerStyle(8);
    g[k][2]->SetMarkerSize(.6);

    g[k][0]->SetMarkerColor(kRed);
    g[k][0]->SetLineColor(kRed);
    g[k][1]->SetMarkerColor(kBlue);
    g[k][1]->SetLineColor(kBlue);
    g[k][2]->SetMarkerColor(kBlack);
    g[k][2]->SetLineColor(kBlack);
  }

  TLegend* legend = new TLegend(0.15, 0.75, 0.3, 0.92, "", "NDC");
  legend->AddEntry(g[0][0],"1 #pi +","lpf");
  legend->AddEntry(g[1][1],"2 #pi +","lpf");
  legend->AddEntry(g[2][2],"3 #pi +","lpf");
  legend->SetFillStyle(0);
  legend->SetBorderSize(0);
  legend->SetTextFont(62);
  ...
  for(int i = 0; i < N_STARGETS; i++){
    ...
    // TLegend* legendNPion = GenTLegendNPion(g);
    legend->Draw();
    c->Print(plotDirectory + Form("Pt_broad_%s-%i.pdf", var, i));
...
void PtbroadeningMultiZhSplit(TString inputDirectory, TString plotDirectory) {
  ...
  c->Print(plotDirectory + "Pt_broad_multiZhC.pdf");
  fsource->Close();
  delete c;
